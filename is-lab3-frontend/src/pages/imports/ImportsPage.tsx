...
const ImportsPage = observer(() => {
  ...
  const onDownload = async (id: number) => {
    try {
      const { blob, fileName } = await ImportsService.downloadFile({ id });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = fileName ?? `import-${id}.txt`;
      ...
      const message = await getApiErrorMessage(e);
      toaster.create({
        title: message ?? 'Не удалось скачать файл импорта',
        type: 'error',
      });
    }
  };
                ...
                <Table.Row key={op.id}>
                  ...
                  <Table.Cell textAlign="end">
                    <HStack>
                      ...
                      <IconButton size={'xs'} variant={'outline'} onClick={() => onDownload(op.id)} disabled={op.status !== 'SUCCEEDED'}>
                        <LuDownload />
                      </IconButton>
                    </HStack>
                  </Table.Cell>
